...
const state = { routes: [], partialRoutes: [],routesfilter:[] }
const getters = {
  routes: (state) => state.routes,
  routesfilter:(state)=>{
    return state.routes.filter(x=>!x.hidden)
  },
...
const actions = {
  resetRoutesfilter() {
    for(let i=0;i<state.routes.length;i++){
      delete state.routes[i].height;
      delete state.routes[i].width;
      delete state.routes[i].select;
      delete state.routes[i].bg;
      delete state.routes[i].arrowRotate
      delete state.routes[i].t
    }
  },
